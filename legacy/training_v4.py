    ...
    def forward(self, questions: List[str], sent_embs: List[torch.Tensor], 
        ...
        for idx, (qv, emb_ex, nbrs, lbl) in enumerate(zip(q_embs, valid_embs, valid_neighbors, valid_labels)):
            try:
                ...
                path_scores = torch.sigmoid(self.path_net(base_inp)).squeeze()
                
                # Handle supporting facts
                if lbl.sum() > 0:
                    supports = torch.nonzero(lbl, as_tuple=False).squeeze(-1)
                    if supports.dim() == 0:
                        supports = supports.unsqueeze(0)
                    valid_supports = supports[supports < path_scores.size(0)]
                    path_scores = path_scores.index_add(0, valid_supports, torch.ones_like(valid_supports, dtype=path_scores.dtype))
                
                # Compute coin amplitudes
                amps = self.coin_net(base_inp)
                amps = amps * (1 + path_scores.unsqueeze(1))
                ...
                for _ in range(self.walk_steps):
                    ...
                    new_state = new_state.view(n_ex, self.k)
...
def evaluate_model(model, dataloader, device, num_batches=None):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(dataloader, desc='Evaluating', total=total_samples)):
            ...
            if not logits_list:  # Skip empty batches
                empty_batches += 1
                continue
            ...
            for logits, lbl in zip(logits_list, labels):
                if lbl.sum() == 0:  # Skip examples with no positive labels
                    continue
                
                # Calculate probabilities and loss
                probs = torch.softmax(logits.float(), dim=0)
                
                # Use binary cross entropy with logits for better gradient flow
                loss = F.binary_cross_entropy_with_logits(logits, lbl, reduction='mean')
                ...
                if not (torch.isnan(loss) or torch.isinf(loss)):
                    ...
                    if len(pred_indices) > 0:
                        precision = len(pred_indices.intersection(true_indices)) / len(pred_indices)
                    else:
                        precision = 0.0
                        
                    if len(true_indices) > 0:
                        recall = len(pred_indices.intersection(true_indices)) / len(true_indices)
                    else:
                        recall = 0.0
            ...
            if valid_examples > 0:
                # Average metrics over valid examples
                batch_loss = batch_loss / valid_examples
                batch_support_f1 = batch_support_f1 / valid_examples
                batch_support_precision = batch_support_precision / valid_examples
                batch_support_recall = batch_support_recall / valid_examples
                
                total_loss += batch_loss
                total_support_f1 += batch_support_f1
                total_support_precision += batch_support_precision
                total_support_recall += batch_support_recall
                total_batches += 1
    ...
    print(f"Evaluation: Valid examples: {total_examples}, Empty batches: {empty_batches}")
...
def main():
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        pbar = tqdm(train_dataloader, desc=f"Epoch {epoch} Training", leave=False)
        for batch in pbar:
        # =====================================================================
            ...
            # Forward pass with autocast
            with autocast(device_type='cuda'):
                # Model forward pass now receives the filtered batch data
                logits_list = model(questions, sent_embs, neighbors, labels)
            
            if not logits_list: # If model returns empty list after internal filtering
                empty_batches += 1
                continue
            ...
            actual_labels = [labels[i] for i, l in enumerate(logits_list)] # Align labels with returned logits

            # Ensure logits_list and actual_labels have the same length
            if len(logits_list) != len(actual_labels):
                 print(f"Warning: Mismatch between logits_list ({len(logits_list)}) and actual_labels ({len(actual_labels)}) lengths. Skipping batch.")
                 empty_batches += 1
                 continue

            for logits, lbl in zip(logits_list, actual_labels):
                # === REMOVE label normalization for BCE loss ===
                if lbl.sum() == 0:  # Skip examples with no positive labels
                    continue
                lbl_sum = lbl.sum()
                if lbl_sum == 0: continue # Double check
                lbl = lbl / lbl_sum 
                # ==============================================
                
                # === CHANGE Loss function ===
                # Calculate probabilities and loss - OLD
                probs = torch.softmax(logits.float(), dim=0)
                loss = F.kl_div(probs.log(), lbl, reduction='batchmean')

                # Calculate loss using BCEWithLogitsLoss - NEW
                # Ensure labels `lbl` are the original 0/1 floats
                # loss = F.binary_cross_entropy_with_logits(logits, lbl, reduction='mean')
                # ==========================
            ...
            if valid_examples_in_batch > 0:
                ...
                # Check gradients before clipping/stepping
                # for name, param in model.named_parameters():
                #     if param.grad is not None:
                #         if torch.isnan(param.grad).any() or torch.isinf(param.grad).any():
                #              print(f"Warning: NaN/Inf gradient found in {name}")
                #              # Option: zero out bad gradients?
                ...
                train_batches += 1
                pbar.set_postfix({'loss': batch_loss.item()}) # Update progress bar
            else:
                empty_batches += 1
