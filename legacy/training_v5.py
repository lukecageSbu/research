    ...
    def forward(self, questions: List[str], sent_embs: List[torch.Tensor], 
        ...
        for idx, (qv, emb_ex, nbrs, lbl) in enumerate(zip(q_embs, valid_embs, valid_neighbors, valid_labels)):
            try:
                ...
                # Compute path scores (still computed, but not used to scale amps or boosted by labels)
                ...
                # Return raw scores (sum of absolute values across k dimensions)
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(dataloader, desc='Evaluating', total=total_samples)):
            ...
            for logits, lbl in zip(logits_list, labels):
                ...
                if lbl.numel() == 0 or logits.numel() == 0 or logits.size(0) != lbl.size(0):
                    print(f"Warning: Skipping example due to size mismatch or empty tensors (logits: {logits.shape}, labels: {lbl.shape})")
                ...
                if not (torch.isnan(loss) or torch.isinf(loss)):
                    ...
                    if lbl.sum() > 0: # Only calculate F1 if positive labels exist
                        ...
                        # Need probabilities for topk comparison, calculate from logits
                        probs = torch.sigmoid(logits.float()) # Sigmoid for BCE scores
            ...
            if valid_examples > 0:
                ...
                batch_loss_avg = batch_loss / len(logits_list) # Average loss over all processed examples
                ...
                if len(logits_list) > 0:
                     batch_loss_avg = batch_loss / len(logits_list)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        # =====================================================================
            ...
            for logits, lbl in zip(logits_list, actual_labels):
                ...
                if lbl.numel() == 0 or logits.numel() == 0 or logits.size(0) != lbl.size(0):
                    print(f"Warning: Skipping loss calculation due to size mismatch or empty tensors (logits: {logits.shape}, labels: {lbl.shape})")
                    continue # Skip if sizes mismatch
