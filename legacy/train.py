...
from utils.batching import extract_graph_labels
import yaml


class HotpotGNNModel(nn.Module):
    """
    Complete GNN model for HotpotQA multi-hop reasoning.
    Combines GraphSAGE encoder with PrefixScorer.
    """
    ...
    def forward(self, batch):
        """Forward pass through complete GNN."""
        # Encode nodes
...
def main():
    ...
    print_memory_usage()
    # Load configuration
    with open(args.config if hasattr(args, 'config') else 'config.yaml', 'r') as f:
